...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_AUDIO] = LAYOUT(
        KC_MUTE, KC_ENT , LT(_HUE, KC_MPRV), LT(_SAT, KC_MPLY), LT(_VAL, KC_MNXT), LT(_MODE, KC_SPC)),
    [_HUE] = LAYOUT(
        RGB_TOG, RGBRST, _______, _______, RGB_HUD, RGB_HUI),
    [_SAT] = LAYOUT(
        _______, _______, SGUI(KC_C), _______, RGB_SAD, RGB_SAI),
    [_VAL] = LAYOUT(
        _______, _______, RGB_VAD, RGB_VAI, _______, SGUI(KC_ENT)),
    [_MODE] = LAYOUT(
        RESET  , WRTROM, RGB_RMOD, RGB_MOD, RGB_TOG, _______),
};
